...
3. Add the following environment variables:
   - `DATABASE_URL`: Your PostgreSQL connection string (e.g., from NeonDB, AWS RDS, etc.)
   - `USE_DAPR`: Set to "false" for Railway deployment (default behavior)
   - `KAFKA_BOOTSTRAP_SERVERS`: Your Kafka broker URL (optional, leave empty to disable Kafka)
4. Deploy the application using Railway's interface

### Important Notes for Railway Deployment:
- The application is configured to work without Kafka by default when `KAFKA_BOOTSTRAP_SERVERS` is not set
- All core features (task management, priorities, tags, search, etc.) work without Kafka
- Event-driven features (notifications, recurring tasks via events) will be disabled without Kafka
- For production use, consider setting up a managed Kafka service like Redpanda Cloud or Confluent Cloud
